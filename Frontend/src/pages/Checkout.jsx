...
export default function Checkout({ cartItems, setCartItems }) {
    ...
    /* =====================================
       2️⃣ Auto-geocode when shipping address fields change
    ===================================== */
    useEffect(() => {
        const { areaStreet, city, state, zipCode, country } = form;
        const queryParts = [areaStreet, city, state, zipCode, country].filter(Boolean);
        const queryString = queryParts.join(', ');

        // Need at least city to attempt geocoding
        if (!city) return;
        ...
        const timer = setTimeout(async () => {
            try {
                ...
                console.log('Checkout geocoding error:', err);
            }
        }, 800);

        return () => clearTimeout(timer);
    }, [form.areaStreet, form.city, form.state, form.zipCode, form.country]);
