...
export default function Profile() {
    ...
    const handleSaveAddress = async () => {
        ...
        try {
            ...
            // Save address — backend will auto-geocode and set lat/lng
            const addressRes = await API.put('/users/address', addressData);

            // Also update auth context with the address and geocoded location
            const profileUpdate = { address: addressData };
            if (addressRes.data?.location?.lat && addressRes.data?.location?.lng) {
                profileUpdate.location = {
                    latitude: addressRes.data.location.lat,
                    longitude: addressRes.data.location.lng
                };
            }
            ...
            if (hasCoords) {
                setMessage("✅ Address & location coordinates updated successfully!");
            } else {
                setMessage("✅ Address saved! Could not determine coordinates — tap 📍 Use Current Location for accurate delivery.");
            }
            setTimeout(() => setMessage(''), 5000);
